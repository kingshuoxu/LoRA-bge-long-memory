...
- **prompt/RAG 之外的第三条长记忆路线**:RAG 检索的是文本、塞进 prompt 让模型现场阅读;这里检索的是"已经学会的能力"——记忆固化在权重里,回答从"边看边答"变成"早已记住",不占 context、零额外 token 成本。
- **记忆变成可管理的文件资产**:每条记忆 = 一个几 MB 的 adapter 文件,可增删、备份、分发、组合、版本化,且完全不动基座——这是 prompt 记忆和全量微调都做不到的工程性质。
- **用对照实验回答了"为什么非要专家隔离"**:同一个 LoRA 顺序续训同样数据,旧批记忆只剩 0~12%;拆成专家后 82~100%。不遗忘来自参数隔离本身,不是任务简单。
- **标定了这条路线的真实边界与上限**:bge 单层路由实测 200 专家 / 1 万条事实零错误;τ∈[0.55,0.95] 安全域内行为不变;同时暴露了"条件反射"边界(换问法可能失配)与训练晚期发散现象——结论可复现、可证伪,0.5B/1.5B 双基座方向一致。
